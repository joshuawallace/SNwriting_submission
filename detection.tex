...
the breakout burst.  
It consists of both detectors currently running and detectors
...
part of the Deep Underground Neutrino Experiment    
 (DUNE; \citealt{deoliviera2015}).  We calculate that an \ar40\ detector of
...
case will be \abt 40. The timing resolution of DUNE depends on its 
...
Scintillation detectors have the advantage that electron-scattering 
...
\tablewidth{0pc}
\tablecolumns{6}
\tablecaption{Approximate Number of $\nu_e$'s Detected in 10 ms Interval during Preshock Neutronization Peak in the No-oscillation Case\label{tab:numdetectionsprebreakout}}
\tablehead{
\colhead{Distance} & \colhead{DUNE} & \colhead{Super-K}
& \colhead{Hyper-K} & \colhead{IceCube}
& \colhead{JUNO} \\ \colhead{(kpc)} & \colhead{} & 
